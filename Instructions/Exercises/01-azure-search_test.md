...
### إنشاء مورد كلام الذكاء الاصطناعي في Azure
 ...
 في مستعرض ويب، افتح مدخل Azure في `https://portal.azure.com`، ثم سجل الدخول باستخدام حساب Microsoft المقترن باشتراك Azure لديك.
 اضغط على زر **&#65291;Create a resource** وابحث عن *بحث عن*، وأنشئ مورد **Azure AI Search** باستخدام الإعدادات التالية:
 ...
 انتظر حتى يكتمل النشر، ثم انتقل إلى المورد المنشور.
 راجع صفحة**نظرة عامة** على الجزء لمورد Azure AI Search في مدخل Microsoft Azure. هنا، يمكنك استخدام واجهة مرئية لإنشاء المكونات المختلفة لحل البحث واختبارها وإدارتها ومراقبتها؛ بما في ذلك مصادر البيانات والفهارس والمفهرسات ومجموعات المهارات.
...
إذا لم يكن لديك بالفعل مورد في اشتراكك، فسيتعين عليك توفير مورد **خدمات الذكاء الاصطناعي في Azure**. سيستخدم حل البحث الخاص بك هذا لإثراء البيانات الموجودة في مخزن البيانات بالرؤى التي تم إنشاؤها بواسطة الذكاء الاصطناعي.
 ...
 ارجع إلى الصفحة الرئيسية لبوابة Azure، ثم حدد الزر **&#65291;Create a resource**، وابحث عن *خدمات الذكاء الاصطناعي في Azure*، وأنشئ مورد **حساب خدمة متعدد لخدمات الذكاء الاصطناعي في Azure** من خلال الإعدادات التالية:
    - **Subscription**: *اشتراكك في Azure*
 ...
 حدد مربعات الاختيار المطلوبة ثم أنشئ المورد.
 انتظر حتى يكتمل النشر، ثم اعرض تفاصيل النشر.

### إنشاء حساب تخزين

 ارجع إلى الصفحة الرئيسية لمدخل Microsoft Azure، ثم حدد الزر **&#65291;Create a resource**، وابحث عن *storage account*، وأنشئ مورد **Storage account** من خلال الإعدادات التالية:
    - **Subscription**: *اشتراكك في Azure*
 ...
 انتظر حتى يكتمل النشر، ثم انتقل إلى المورد المنشور.
 في **صفحة نظرة عامة**، لاحظ **معرف الاشتراك** - يحدد هذا الاشتراك الذي يتم توفير حساب التخزين فيه.
 في صفحة **الوصول إلى المفاتيح**، لاحظ أنه تم إنشاء مفتاحين لحساب التخزين الخاص بك. ثم حدد **إظهار المفاتيح** لعرض المفاتيح.
...
> **تلميح**: إذا نسخت بالفعل مستودع **mslearn-knowledge-mining**، فافتحه في Visual Studio code. وإلا فاتبع هذه الخطوات لاستنساخه إلى بيئة تطويرك.
 ...
 افتح لوحة (SHIFT+CTRL+P) وشغّل **Git: استنسخ الأمر ** لاستنساخ مستودع `https://github.com/MicrosoftLearning/mslearn-knowledge-mining` إلى مجلد محلي (لا يُهم أي مجلد).
 عند استنساخ المستودع، افتح المجلد في تعليمة Visual Studio البرمجية.
 انتظر حتى تثبيت ملفات إضافية لدعم مشاريع التعليمات البرمجية C# في المستودع.
...
الآن بعد أن أصبح لديك الموارد المطلوبة، يمكنك تحميل بعض المستندات إلى حساب Azure Storage الخاص بك.
 ...
 في Visual Studio Code، في جزء** Explorer**، قم بتوسيع مجلد**مجلد Labfiles\01-azure-search** وحدد **UploadDocs.cmd**.
 قم بتحرير ملف الدُفعات لاستبدال العناصر النائبة ** YOUR_SUBSCRIPTION_ID **،**وYOUR_AZURE_STORAGE_ACCOUNT_NAME** و**YOUR_AZURE_STORAGE_KEY** بمعرف الاشتراك المناسب واسم حساب تخزين Azure وقيم مفتاح حساب تخزين Azure لحساب التخزين الذي قمت بإنشائه مسبقًا.
 احفظ التغييرات، ثم انقر بزر الماوس الأيمن فوق المجلد** 01-azure-search** وافتح محطة طرفية متكاملة.
 أدخل الأمر التالي لتسجيل الدخول إلى اشتراك Azure باستخدام Azure CLI.
 ...
 أدخل الأمر التالي لتشغيل ملف الدُفعات. سيؤدي ذلك إلى إنشاء حاوية كائن ثنائي كبير الحجم في حساب التخزين الخاص بك وتحميل المستندات في مجلد **البيانات** إليها.
...
الآن بعد أن أصبح لديك المستندات في مكانها، يمكنك إنشاء حل بحث عن طريق فهرستها.
 ...
 في مدخل Microsoft Azure، استعرض وصولاً إلى مورد بحث الذكاء الاصطناعي في Azure. ثم، في صفحة **النظرة العامة،** حدد **استيراد البيانات**.
 في صفحة **الاتصال ببياناتك**، في قائمة **مصدر البيانات**، حدد **Azure Blob Storage**. ثم أكمل تفاصيل مخزن البيانات بالقيم التالية:
 ...
 تابع إلى الخطوة التالية *إضافة مهارات معرفية*.
 في قسم **إرفاق خدمات الذكاء الاصطناعي في Azure** حدد مورد خدمات الذكاء الاصطناعي في Azure.
 في القسم **إضافة إثراءات**:
 ...
 تحقق مرة أخرى من اختياراتك (قد يكون من الصعب تغييرها لاحقاً). ثم انتقل إلى الخطوة التالية (*تخصيص الفهرس الهدف*).
 غيّر **اسم الفهرس** إلى **margies-index**.
 تأكد من تعيين **المفتاح** على **metadata_storage_path** واترك **اسم المقترح فارغاً ووضع البحث ****على الوضع الافتراضي**.
 قم بإجراء التغييرات التالية على حقول الفهرس، مع ترك جميع الحقول الأخرى بإعداداتها الافتراضية (**مهم**: قد تحتاج إلى التمرير إلى اليمين لرؤية الجدول بالكامل):
 ...
 تحقق جيدًا من اختياراتك، مع إيلاء اهتمام خاص للتأكد من تحديد الخيارات الصحيحة **Retrievable**،**وFilterable**،**وSortable**،**وFacetable**و **Searchable** لكل حقل (قد يكون من الصعب تغييرها لاحقًا). ثم تابع إلى الخطوة التالية (*إنشاء مفهرس*).
 غيّر **اسم المفهرس** إلى **margies-index**.
 اترك **الجدول** معيناً على **مرة واحدة**.
 قم بتوسيع الخيارات **المتقدمة**، وتأكد من تحديد الخيار **مفاتيح الترميز Base-64** (تجعل مفاتيح الترميز الفهرس أكثر كفاءة بشكل عام).
 حدد **إرسال** لإنشاء مصدر البيانات ومجموعة المهارات والفهرس والمفهرس. يتم تشغيل المفهرس تلقائياً ويقوم بتشغيل خط أنابيب الفهرسة، والذي:
 ...
 على الجانب الأيمن، اعرض صفحة **المُفهرسات**، والتي يجب أن تظهر **margies-indexer** الذي تم إنشاؤه حديثًا. انتظر بضع دقائق، ثم انقر فوق **&orarr;تحديث** حتى تشير **الحالة** إلى نجاح.

## البحث في الفهرس

الآن بعد أن أصبح لديك فهرس، يمكنك البحث فيه.

 في أعلى صفحة **نظرة عامة**، لمورد Azure AI Search خاصتك، حدد **مستكشف البحث**.
 في مستكشف البحث، في مربع **سلسلة الاستعلام**، أدخل `*` (علامة نجمية واحدة)، ثم حدد **بحث**.
 ...
 في قائمة **العرض**، حدد **طريقة عرض JSON** ولاحظ ظهور طلب JSON للبحث، مثل هذا:
 ...
 قم بتعديل طلب JSON ليتضمن معلمة **العد** كما هو موضح هنا:
 ...
 أرسل البحث المعدل. هذه المرة، تتضمن النتائج حقلًا**@odata.count** في أعلى النتائج يشير إلى عدد المستندات التي تم إرجاعها بواسطة البحث.
 ...
 الآن جرب سلسلة الاستعلام التالية:
...
### الحصول على نقطة النهاية والمفتاح لمورد Azure AI Search resource خاصتك
 ...
 في مدخل Microsoft Azure، ارجع إلى صفحة** نظرة عامة** لمورد Azure AI Search خاصتك، وفي القسم العلوي من الصفحة، ابحث عن **عنوان URL** للمورد لديك (الذي يشبه **https://resource_name.search.windows.net**) وانسخه إلى الحافظة.
 في Visual Studio Code، في جزء Explorer، قم بتوسيع المجلد ** 01-azure-search** ومجلده الفرعي ** modify-search**، وحدد **modify-search.cmd** لفتحه. ستستخدم ملف البرنامج النصي هذا لتشغيل أوامر * cURL* التي ترسل JSON إلى واجهة REST لخدمة الذكاء الاصطناعي في Azure.
 في **modify-search.cmd**، استبدل العنصر النائب** YOUR_SEARCH_URL** بعنوان URL الذي نسخته إلى الحافظة.
 في بوابة Azure، في قسم **الإعدادات**، اعرض صفحة **المفاتيح** لمورد Azure AI Search الخاص بك، وانسخ **مفتاح المسؤول الأساسي** إلى الحافظة.
 في Visual Studio Code، استبدل العنصر النائب ** YOUR_ADMIN_KEY**بالمفتاح الذي نسخته إلى الحافظة.
 احفظ التغييرات على **modify-search.cmd** (ولكن لا تقم بتشغيلها بعد!)

### مراجعة مجموعة المهارات وتعديلها

 في Visual studio Code، في مجلد** modify-search**، افتح **skillset.json**. يظهر هذا تعريف JSON لمجموعة **margies-skillset**.
 في أعلى تعريف مجموعة المهارات، لاحظ كائن ** cognitiveServices**، والذي يستخدم لتوصيل مورد خدمات الذكاء الاصطناعي في Azure بمجموعة المهارات.
 في بوابة Azure، افتح مورد Azure AI Services (<u>ليس</u> مورد Azure AI Search الخاص بك!) وفي قسم **إدارة الموارد**، اعرض صفحة **المفاتيح ونقاط النهاية** الخاصة به. ثم انسخ **المفتاح 1**إلى الحافظة.
 في Visual Studio Code، في **skillset.json**، استبدل العنصر النائب** YOUR_COGNITIVE_SERVICES_KEY** بمفتاح خدمات الذكاء الاصطناعي في Azure الذي نسخته إلى الحافظة.
 قم بالتمرير عبر ملف JSON، مع ملاحظة أنه يتضمن تعريفات للمهارات التي أنشأتها باستخدام واجهة مستخدم Azure AI Search في مدخل Microsoft Azure. في أسفل قائمة المهارات، تمت إضافة مهارة إضافية باستخدام التعريف التالي:
 ...
 احفظ التغييرات التي أجريتها على**skillset.json**.

### مراجعة الفهرس وتعديله

 في Visual studio Code، في مجلد** modify-search**، افتح **index.json**. يظهر هذا تعريف JSON لـ **margies-index**.
 قم بالتمرير عبر الفهرس وشاهد تعريفات الحقول. تستند بعض الحقول إلى بيانات التعريف والمحتوى في المستند المصدر، وتستند الحقول الأخرى على نتائج المهارات في مجموعة المهارات.
 في نهاية قائمة الحقول التي قمت بتعريفها في مدخل Microsoft Azure، لاحظ أنه تمت إضافة حقلين إضافيين:
 ...
 سيتم استخدام حقل**التوجه** لإضافة الإخراج من مهارة** get-sentiment** التي تمت إضافتها إلى مجموعة المهارات. سيتم استخدام حقل**url** لإضافة عنوان URL لكل مستند مفهرس إلى الفهرس، استنادًا إلى القيمة** metadata_storage_path** المستخلصة من مصدر البيانات. لاحظ أن الفهرس يتضمن بالفعل حقل ** metadata_storage_path**، ولكنه يستخدم كمفتاح فهرس والمرمز باستخدام Base-64، ما يجعله فعالاً كمفتاح ولكنه يتطلب من تطبيقات العميل فك ترميزه إذا كانوا يريدون استخدام قيمة URL الفعلية كحقل. تؤدي إضافة حقل ثان للقيمة غير المشفرة إلى حل هذه المشكلة.

### مراجعة المفهرس وتعديله

 في Visual studio Code، في مجلد** modify-search**، افتح **indexer.json**. يعرض هذا تعريف JSON لمفهرس **margies-indexer**، الذي يعين الحقول المستخلصة من محتوى المستند وبيانات التعريف (في قسم** fieldMappings**)، والقيم المستخلصة بواسطة المهارات في مجموعة المهارات (في قسم** outputFieldMappings**)، إلى الحقول في الفهرس.
 في قائمة ** fieldMappings**، لاحظ تعيين قيمة** metadata_storage_path** إلى حقل المفتاح المشفر باستخدام base-64. تم إنشاء هذا عند تعيين **metadata_storage_path** كمفتاح وتحديد خيار لترميز المفتاح في مدخل Microsoft Azure. بالإضافة إلى ذلك، يقوم التعيين الجديد بتعيين نفس القيمة بشكل صريح إلى حقل عنوان **url**، ولكن بدون ترميز Base-64:
 ...
 راجع قسم** ouputFieldMappings**، الذي يعين المخرجات من المهارات في مجموعة المهارات إلى حقول الفهرس. يعكس معظم ذلك الخيارات التي قمت بها في واجهة المستخدم، ولكن تمت إضافة التعيين التالي لتعيين قيمة ** sentimentLabel** المستخلصة من مهارة التوجه الخاصة بك إلى حقل** التوجه **الذي أضفته إلى الفهرس:
